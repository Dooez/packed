...
public:
    ...
    void fft_internal(T2* source)
        requires std::same_as<T2, float> && (SubSize == pcx::dynamic_size)
    {
    ...
    template<typename T2>
    void fft_internal(T2* dest, const T2* source)
        requires std::same_as<T2, float>
    {
        const auto sq2 = wnk(8, 1);
