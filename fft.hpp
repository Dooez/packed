...
public:
    ...
    template<typename VAllocator, std::size_t VPackSize>
    void unsorted_re(pcx::vector<T, VAllocator, VPackSize>& vector) {
        assert(size() == vector.size());
        fftu_internal_re<VPackSize>(vector.data());
    };
    template<typename VAllocator, std::size_t VPackSize>
    void unsorted_op(pcx::vector<T, VAllocator, VPackSize>& vector) {
        if (size() != vector.size()) {
            throw(std::string("Size, which is ") + std::to_string(size()) + "not equal vector.size() " +
                  std::to_string(vector.size()));
        }
        fftu_internal_op<VPackSize>(vector.data());
    };
    template<typename VAllocator, std::size_t VPackSize>
    void unsorted_fi(pcx::vector<T, VAllocator, VPackSize>& vector)
        requires(Size != pcx::dynamic_size)
    {
        assert(size() == vector.size());
        fftu_internal_op_fi<VPackSize>(vector.data());
    };
    template<typename VAllocator>
...
private:
    ...
    const std::vector<real_type, allocator_type>           m_twiddles_unsorted_re =
        get_twiddles_unsorted_re(size(), sub_size(), allocator_type());
...
public:
    ...
    template<std::size_t PData>
    void fftu_internal_op(float* data) {
        auto* twiddle_ptr = m_twiddles_unsorted.data();
        if (log2i(size() / sub_size()) % 2 == 0) {
            unsorted_subtransform_recursive_op<PData, PData, true>(data, size(), twiddle_ptr);
        } else {
            constexpr auto PTform = std::max(PData, reg_size);
            ...
            twiddle_ptr =
                unsorted_subtransform_recursive_op<PData, PTform, true>(data, size() / 2, twiddle_ptr);
            unsorted_subtransform_recursive_op<PData, PTform>(
                avx::ra_addr<PTform>(data, size() / 2), size() / 2, twiddle_ptr);
        }
    }

    template<std::size_t PData>
    void fftu_internal_op_fi(float* data) {
        auto* twiddle_ptr = m_twiddles_unsorted.data();
        if (log2i(size() / sub_size()) % 2 == 0) {
            unsorted_subtransform_recursive_op_fi<PData, PData, Size, true>(data, twiddle_ptr);
        } else {
            constexpr auto PTform = std::max(PData, reg_size);
            ...
            twiddle_ptr =
                unsorted_subtransform_recursive_op_fi<PData, PTform, Size / 2, true>(data, twiddle_ptr);
            unsorted_subtransform_recursive_op_fi<PData, PTform, Size / 2>(
                avx::ra_addr<PTform>(data, size() / 2), twiddle_ptr);
        }
    }

    template<std::size_t PData>
    void fftu_internal_re(float* data) {
        auto* twiddle_ptr = m_twiddles_unsorted_re.data();
        if (log2i(size() / sub_size()) % 2 == 0) {
            unsorted_subtransform_recursive_re<PData, PData>(data, size(), twiddle_ptr);
        } else {
            constexpr auto PTform = std::max(PData, reg_size);
            ...
            twiddle_ptr = unsorted_subtransform_recursive_re<PData, PTform>(data, size() / 2, twiddle_ptr);
            unsorted_subtransform_recursive_re<PData, PTform>(
                avx::ra_addr<PTform>(data, size() / 2), size() / 2, twiddle_ptr);
        }
    }
    ...
    inline auto unsorted_subtransform_op(float* data, std::size_t size, const float* twiddle_ptr) -> const
        float* {
        constexpr auto PTform = std::max(PSrc, reg_size);
    ...
    template<std::size_t PDest, std::size_t PSrc, bool First = false>
    inline auto unsorted_subtransform_recursive_op(float* data, std::size_t size, const float* twiddle_ptr)
        -> const float* {
        if (size <= sub_size()) {
            return unsorted_subtransform_op<PDest, PSrc, First>(data, size, twiddle_ptr);
        } else {
            constexpr auto PTform = std::max(PSrc, reg_size);
            ...
            if constexpr (First) {
                ...
                for (std::size_t i_group = 0; i_group < size / 4 / reg_size; ++i_group) {
                    auto offset = i_group * reg_size;
                    node4_dif<PTform, PSrc>(data, size, offset);
                }
            } else {
                reg_t tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
                reg_t tw1 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
            ...
            twiddle_ptr =
                unsorted_subtransform_recursive_op<PDest, PTform, First>(data, size / 4, twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_op<PDest, PTform>(
                avx::ra_addr<PTform>(data, size / 4), size / 4, twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_op<PDest, PTform>(
                avx::ra_addr<PTform>(data, size / 2), size / 4, twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_op<PDest, PTform>(
    ...
    template<std::size_t PDest, std::size_t PSrc, std::size_t TSize, bool First = false>
    inline auto unsorted_subtransform_op(float* data, const float* twiddle_ptr) -> const float* {
        constexpr auto PTform = std::max(PSrc, reg_size);

        using reg_t = avx::cx_reg<float>;

        std::size_t l_size   = TSize;
        std::size_t n_groups = 1;

        if constexpr (PSrc < PTform) {
            if constexpr (TSize > reg_size * 8) {
                if constexpr (First) {
                    twiddle_ptr += 6;
                    for (std::size_t i = 0; i < l_size / reg_size / 4; ++i) {
                        auto offset = i * reg_size;
        ...
        if (l_size == reg_size * 4) {
            for (std::size_t i_group = 0; i_group < TSize / reg_size / 4; ++i_group) {
                reg_t tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
    ...
    template<std::size_t PDest, std::size_t PSrc, std::size_t TSize, bool First = false>
    inline auto unsorted_subtransform_recursive_op_fi(float* data, const float* twiddle_ptr) -> const float* {
        if (TSize <= sub_size()) {
            return unsorted_subtransform_op<PDest, PSrc, TSize, First>(data, twiddle_ptr);
        } else {
            constexpr auto PTform = std::max(PSrc, reg_size);
            ...
            if constexpr (First) {
                twiddle_ptr += 6;
                for (std::size_t i_group = 0; i_group < TSize / 4 / reg_size; ++i_group) {
                    auto offset = i_group * reg_size;
                    node4_dif<PTform, PSrc>(data, TSize, offset);
                }
            } else {
                reg_t tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
                reg_t tw1 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
                ...
                for (std::size_t i_group = 0; i_group < TSize / 4 / reg_size; ++i_group) {
                    auto offset = i_group * reg_size;
                    node4_dif<PTform, PSrc>(data, TSize, offset, tw0, tw1, tw2);
                }
            }
            twiddle_ptr =
                unsorted_subtransform_recursive_op_fi<PDest, PTform, TSize / 4, First>(data, twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_op_fi<PDest, PTform, TSize / 4>(
                avx::ra_addr<PTform>(data, TSize / 4), twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_op_fi<PDest, PTform, TSize / 4>(
                avx::ra_addr<PTform>(data, TSize / 2), twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_op_fi<PDest, PTform, TSize / 4>(
                avx::ra_addr<PTform>(data, TSize / 4 * 3), twiddle_ptr);
            return twiddle_ptr;
        }
    };
    ...
    template<std::size_t PDest, std::size_t PSrc>
    inline auto unsorted_subtransform_recursive_re(float* data, std::size_t size, const float* twiddle_ptr)
        -> const float* {
        if (size <= sub_size()) {
            return unsorted_subtransform_re<PDest, PSrc>(data, size, twiddle_ptr);
        } else {
            constexpr auto PTform = std::max(PSrc, reg_size);
            ...
            twiddle_ptr = unsorted_subtransform_recursive_re<PDest, PTform>(    //
                data,
                size / 4,
                twiddle_ptr);
            twiddle_ptr = unsorted_subtransform_recursive_re<PDest, PTform>(
...
private:
    ...
    static auto get_twiddles_unsorted_re(std::size_t fft_size, std::size_t sub_size, allocator_type allocator)
        -> std::vector<T, allocator_type> {
        auto twiddles = std::vector<T, allocator_type>(allocator);

        std::size_t l_size = 2;
        if (log2i(fft_size / sub_size) % 2 == 0) {
            insert_tw_unsorted_re(fft_size, l_size, sub_size, 0, twiddles);
        } else {
            auto tw0 = wnk(l_size, reverse_bit_order(0, log2i(l_size / 2)));
            ...
            insert_tw_unsorted_re(fft_size, l_size * 2, sub_size, 0, twiddles);
            insert_tw_unsorted_re(fft_size, l_size * 2, sub_size, 1, twiddles);
        }
        return twiddles;
    }

    static void insert_tw_unsorted_re(std::size_t                             fft_size,
                                      std::size_t                             l_size,
                                      std::size_t                             sub_size,
                                      std::size_t                             i_group,
                                      std::vector<real_type, allocator_type>& twiddles) {
        if ((fft_size / l_size) < sub_size) {
            ...
            insert_tw_unsorted_re(fft_size, l_size, sub_size, i_group + 0, twiddles);
            insert_tw_unsorted_re(fft_size, l_size, sub_size, i_group + 1, twiddles);
            insert_tw_unsorted_re(fft_size, l_size, sub_size, i_group + 2, twiddles);
            insert_tw_unsorted_re(fft_size, l_size, sub_size, i_group + 3, twiddles);
        }
    }
};
