...
/**
  ...
  * @param args any combination of std::array<T*, 4> src, std::array<avx::cx_reg<T>, 3> tw, avx::reg_t<T> scaling
  */
template<typename T, std::size_t PDest, std::size_t PSrc, bool ConjTw, bool Reverse, typename... Args>
...
public:
    ...
    void fftu_internal(float* dest, const float* source, std::size_t source_size = 0) {
        ...
        if (source_size == 0) {
            ...
            if (log2i(size() / (reg_size * 4)) % 2 == 0) {
                ...
                for (std::size_t i_group = 0; i_group < size() / 2 / reg_size; ++i_group) {
                    node2_dif_along_fill0<PTform, PSrc>(
                        dest, source, size(), source_size, i_group * reg_size);
                }
    ...
    inline auto subtransform_recursive(T* data, std::size_t size) -> const T* {
        if (size <= sub_size()) {
            ...
            for (std::size_t i_group = 0; i_group < n_groups; ++i_group) {
                ...
                twiddle_ptr += reg_size * 6;
                node4_dit_along<PDest, PTform, Inverse, Scale>(
                    data, size / 2, i_group * reg_size, tw0, tw1, tw2, scaling);
            }
            return twiddle_ptr;
        }
    };
    ...
    inline auto unsorted_subtransform_recursive(T* data, std::size_t size, const T* twiddle_ptr) -> const T* {
        ...
        if constexpr (First) {
            ...
            for (std::size_t i_group = 0; i_group < size / 4 / reg_size; ++i_group) {
                node4_dif_along<PTform, PSrc>(data, size, i_group * reg_size);
            }
        } else {
            avx::cx_reg<T> tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
            avx::cx_reg<T> tw1 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
            avx::cx_reg<T> tw2 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
    ...
    template<std::size_t PDest, std::size_t PSrc, bool First = false, bool BitReverse = true>
    inline auto unsorted_subtransform_fill0(float*       dest,
                                            const float* src,
                                            std::size_t,
                                            std::size_t  size,
        ...
        if (l_size > reg_size * 8) {
            if constexpr (First) {
                twiddle_ptr += 6;
                for (std::size_t i = 0; i < l_size / reg_size / 4; ++i) {
                    node4_dif_along_fill0<PTform, PSrc>(dest, src, l_size, data_size, i * reg_size);
                }
            } else {
                reg_t tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
                ...
                for (std::size_t i = 0; i < l_size / reg_size / 4; ++i) {
                    node4_dif_along_fill0<PTform, PSrc>(
                        dest, src, l_size, data_size, i * reg_size, {tw0, tw1, tw2});
                }
            }
            l_size /= 4;
            n_groups *= 4;
            ...
            reg_t tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
            for (std::size_t i = 0; i < l_size / reg_size / 2; ++i) {
                node2_dif_along_fill0<PTform, PSrc>(dest, src, l_size, data_size, i * reg_size, tw0);
            }
        ...
        while (l_size > reg_size * 8) {
            ...
            if constexpr (First) {
                twiddle_ptr += 6;
                for (std::size_t i = 0; i < l_size / reg_size / 4; ++i) {
                    node4_dif_along<PTform, PTform>(src, l_size, i * reg_size);
                }
                ++i_group;
            }
            for (; i_group < n_groups; ++i_group) {
                ...
                auto* group_ptr = avx::ra_addr<PTform>(src, i_group * l_size);
        ...
        for (std::size_t i_group = 0; i_group < size / reg_size / 4; ++i_group) {
            ...
            auto* ptr0 = avx::ra_addr<PTform>(src, reg_size * (i_group * 4));
            auto* ptr1 = avx::ra_addr<PTform>(src, reg_size * (i_group * 4 + 1));
            auto* ptr2 = avx::ra_addr<PTform>(src, reg_size * (i_group * 4 + 2));
            auto* ptr3 = avx::ra_addr<PTform>(src, reg_size * (i_group * 4 + 3));
    ...
    inline auto unsorted_subtransform_recursive_fill0(T*          dest,    //
        ...
        if (size <= sub_size()) {
            return unsorted_subtransform_fill0<PDest, PSrc, First, BitReverse>(dest, src, size, twiddle_ptr);
        }
        constexpr auto PTform = std::max(PSrc, reg_size);
        if constexpr (First) {
            twiddle_ptr += 6;
            for (std::size_t i_group = 0; i_group < size / 4 / reg_size; ++i_group) {
                node4_dif_along_fill0<PTform, PSrc>(dest, src, size, data_size, i_group * reg_size);
            }
        } else {
            avx::cx_reg<T> tw0 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
            avx::cx_reg<T> tw1 = {avx::broadcast(twiddle_ptr++), avx::broadcast(twiddle_ptr++)};
    ...
    inline auto unsorted_subtransform_recursive_inverse(T* data, std::size_t size, const T* twiddle_ptr)
        ...
        if constexpr (First) {
            twiddle_ptr -= 6;
            for (std::size_t i_group = 0; i_group < size / 4 / reg_size; ++i_group) {
                node4_dif_along<PTform, PSrc, true>(data, size, i_group * reg_size);
            }
        } else {
            twiddle_ptr -= 6;
            avx::cx_reg<T> tw0 = {avx::broadcast(twiddle_ptr + 0), avx::broadcast(twiddle_ptr + 1)};
            avx::cx_reg<T> tw1 = {avx::broadcast(twiddle_ptr + 2), avx::broadcast(twiddle_ptr + 3)};
            avx::cx_reg<T> tw2 = {avx::broadcast(twiddle_ptr + 4), avx::broadcast(twiddle_ptr + 5)};
            for (std::size_t i_group = 0; i_group < size / 4 / reg_size; ++i_group) {
                node4_dif_along<PTform, PSrc, true>(data, size, i_group * reg_size, tw0, tw1, tw2);
            }
        }
        return twiddle_ptr;
    };
    ...
    inline void
    node2(T* data, std::size_t l_size, std::size_t offset, avx::cx_reg<T> tw0, ScaleT scaling = ScaleT{}) {
        constexpr auto    PLoad = std::max(PSrc, reg_size);
        std::array<T*, 2> ptrs  = {
        ...
        if constexpr (IScale) {
            internal::fft::node2<T, PDest, PSrc, Inverse, false>(ptrs, tw0, scaling);
        } else {
            internal::fft::node2<T, PDest, PSrc, Inverse, false>(ptrs, tw0);
        }
    }
    ...
    template<std::size_t PDest, std::size_t PSrc = PDest, bool Reverse = false>
    inline void node2_dif_along_fill0(    //
        T*             dest,
        const T*       src,
        std::size_t    l_size,
        std::size_t    data_size,
        ...
        if (offset + l_size / 2 + avx::reg<T>::size <= data_size) {
            std::array<const T*, 2> src{
                avx::ra_addr<PLoad>(src, offset),
                avx::ra_addr<PLoad>(src, offset + l_size / 2),
            };
            internal::fft::node2<T, PDest, PSrc, false, Reverse>(dst, src, tw);
        } else {
            ...
            for (uint i = 0; i < 2; ++i) {
                std::ptrdiff_t diff = data_size - offset + l_size / 2 * i;
                if (diff >= avx::reg<T>::size) {
                    src[i] = avx::ra_addr<PLoad>(src, offset + l_size / 2 * i);
                } else if (diff >= 0) {
                    std::array<T, avx::reg<T>::size * 2> align{};
                    for (; diff >= 0; --diff) {
                        align[diff] = *(src + pidx<PSrc>(offset + l_size / 2 * i + diff));
                        align[diff + avx::reg<T>::size] =
                            *(src + pidx<PSrc>(offset + l_size / 2 * i + diff) + PSrc);
                    }
                    src[i] = align.data();
                    internal::fft::node2<T, PDest, PSrc, false, Reverse>(dst, src, tw);
                    return;
    ...
    template<std::size_t PDest, std::size_t PSrc = PDest, bool Reverse = false>
    inline void node4_dif_along_fill0(    //
        T*                            dest,
        const T*                      src,
        std::size_t                   l_size,
        std::size_t                   data_size,
        ...
        if (offset + l_size / 4 * 3 + avx::reg<T>::size <= data_size) {
            std::array<const T*, 4> src{
                avx::ra_addr<PLoad>(src, offset),
                avx::ra_addr<PLoad>(src, offset + l_size / 4),
                avx::ra_addr<PLoad>(src, offset + l_size / 2),
            ...
            for (uint i = 0; i < 4; ++i) {
                ...
                std::ptrdiff_t diff = data_size - offset + l_size / 4 * i;
                if (diff >= avx::reg<T>::size) {
                    src[i] = avx::ra_addr<PLoad>(src, l_offset);
                } else if (diff >= 0) {
                    std::array<T, avx::reg<T>::size * 2> align{};
                    for (; diff >= 0; --diff) {
                        align[diff]                     = *(src + pidx<PSrc>(l_offset + diff));
                        align[diff + avx::reg<T>::size] = *(src + pidx<PSrc>(l_offset + diff) + PSrc);
                    }
                    src[i] = align.data();
                    internal::fft::node4<T, PDest, PSrc, false, Reverse>(dst, src, tw);
                    return;
    ...
    inline void node4_dif_along_fill0(    //
        ...
        if (offset + l_size / 4 * 3 + avx::reg<T>::size <= data_size) {
            ...
            internal::fft::node4<T, PDest, PSrc, false, Reverse>(dst, src);
        };
    }
    template<std::size_t PDest, std::size_t PSrc = PDest, bool Reverse = false>
    inline void node8_dif_along_fill0(    //
        ...
        if (offset + l_size / 8 * 7 + avx::reg<T>::size <= data_size) {
            std::array<const T*, 8> src{
                avx::ra_addr<PLoad>(src, offset),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 1),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 2),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 3),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 4),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 5),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 6),
                avx::ra_addr<PLoad>(src, offset + l_size / 8 * 7),
            };
            ...
            for (uint i = 0; i < 8; ++i) {
                ...
                std::ptrdiff_t diff = data_size - l_offset;
                if (diff >= avx::reg<T>::size) {
                    src[i] = avx::ra_addr<PLoad>(src, l_offset);
                } else if (diff >= 0) {
                    std::array<T, avx::reg<T>::size * 2> align{};
